...
class Sequence {
public:
    virtual T Get(size_t index) const = 0;//виртуальная функция которую необходимо будет переопределять в наследниках
...
public:
    ...
    T Get(size_t index) const override;//override - переопределение функции в наследнике 
